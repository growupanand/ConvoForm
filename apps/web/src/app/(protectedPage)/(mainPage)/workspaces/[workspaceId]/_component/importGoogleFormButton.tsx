...
export default function ImportGoogleFormButton() {
  const { getAccessToken, isAuthenticating, accessToken } = useGoogleAuth();
  ...
  const googleDriveFormsFilesQuery = api.google.getDriveFormsFiles.useQuery(
    { accessToken: accessToken || "" }, // Empty token initially
    {
      enabled: !!accessToken && accessToken !== "", // Don't fetch automatically
    },
  );
  ...
  const fetchGoogleForms = async () => {
    try {
      /**
       * We only need to fetch the access token once initially,
       * then googleDriveFormsFilesQuery will automatically refetch
       */
