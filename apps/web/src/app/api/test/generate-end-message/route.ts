...
curl -X POST http://localhost:3000/api/test/generate-end-message \
...
import { generateEndMessage } from "@convoform/ai";
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("GenerateEndMessage test error:", error);
    return NextResponse.json(
      {
        error: "Failed to generate end message",
...
export async function GET() {
  return NextResponse.json({
    message: "GenerateEndMessage endpoint is ready for testing",
    usage: {
      method: "POST",
